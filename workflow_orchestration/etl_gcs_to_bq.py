...
def etl_gcs_to_bq(year: int, month: int, colour: str):
...
@flow(log_prints=True)
def etl_parent_flow(
    months: list[int] = [2, 3], year: int = 2019, colour: str = "yellow"
):
    l = 0
    for month in months:
        etl = etl_gcs_to_bq(year, month, colour)
        etl
        l = l + etl
        
    print(f"Processed {l} rows")
